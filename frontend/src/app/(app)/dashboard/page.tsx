...
export default function Dashboard() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/dashboard/summary`).then(res => res.json()),
      fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/dashboard/timeseries`).then(res => res.json())
    ...
    const ws = new WebSocket(`${process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:8000'}/api/v1/dashboard/live`)
    ws.onmessage = (event) => {
  ...
  const runSimulation = async () => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      try {
        await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/intercept`, {
          method: "POST",
    ...
    setSimulating(false)
    // Refresh stats
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000'}/api/v1/dashboard/summary`).then(res => res.json()).then(setSummary)
  }
